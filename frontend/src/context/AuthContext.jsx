...
import { createDummyToken } from "../services/dummyData";
    ...
    const login = async (username, password) => {
        try {
            ...
            if (username === "admin" && password === "admin") {
                console.warn("Backend unavailable – using demo login (admin/admin)");
                const dummyAccess = createDummyToken();
                localStorage.setItem("accessToken", dummyAccess);
                localStorage.setItem("refreshToken", "dummy-refresh-token");

                const decoded = jwtDecode(dummyAccess);
                setUser({ username: decoded.username || decoded.user_id, ...decoded });
                return { success: true };
            }
